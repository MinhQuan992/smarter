...
class QuestionService {
  ...
  Future<void> addUserQuestion(UserQuestionRequest userQuestionRequest) async {
    String? token = await _flutterSecureStorage.read(key: "token");
    await http.post(Uri.parse("${constants.baseUrl}/questions/user-questions"),
        headers: {
          "Authorization": "Bearer $token",
          "Content-Type": "application/json"
        },
        body: json.encode(userQuestionRequest));
  }

  Future<void> updateUserQuestion(
      int questionId, UserQuestionRequest userQuestionRequest) async {
    String? token = await _flutterSecureStorage.read(key: "token");
    await http.put(
        Uri.parse("${constants.baseUrl}/questions/user-questions/$questionId"),
        headers: {
          "Authorization": "Bearer $token",
          "Content-Type": "application/json"
        },
  ...
  Future<void> deleteUserQuestion(int questionId) async {
    String? token = await _flutterSecureStorage.read(key: "token");
    await http.delete(
      Uri.parse("${constants.baseUrl}/questions/user-questions/$questionId"),
      headers: {
